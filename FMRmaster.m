...
% This is the master copy, defecate yourself in the warm bask of its glory.
% This m e a t y b o i does not require you to enter the midpoint, and has
% sin/cos built in so ONLY CHANGE THE FIELD MODULUS. Also generates a .mat
% file of FMR slice 'FMRslice.mat'
classdef FMRmaster < hgsetget % subclass hgsetget
...
properties
    ...
    Hprompt;
    H = [0 0 0];
    currentpath = '/OOMMF/projects/Matlab';
    % damping constant
    alpha = 0.001
...
methods
    ...
    function calcDeflection(obj) 
        ...
        addpath(genpath('/OOMMF/projects/Matlab'));
        cd (path);
        obj.Hprompt = input('SHOW US WHAT YOU GOT (for your field value, Oe) ');
        obj.H = [obj.Hprompt*cosd(3) obj.Hprompt*sind(3) 0];
        obj.Hprompt = [];
    ...
    function calcDemagTensor(obj,varargin)
        directory = '/OOMMF/projects/Matlab';
        test = strcmp(pwd,directory);
        if test == 1
    ...
    function calcFreq(obj)
        ...
        obj.freq = obj.gamma*sqrt((H+4*pi*obj.Ms.*(obj.Nxx-obj.Nzz)).*...
            (H+4*pi*obj.Ms.*(obj.Nyy-obj.Nzz)))/(2*pi);
        
        for x = 1:size(obj.freq,1)
            for y = 1:size(obj.freq,2)
                %for z = 1:size(obj.H,3)
        
                    if real(obj.freq(x,y,1,1)) == 0
                        obj.freq(x,y,1,1) = 1i*obj.freq(x,y,1,1);
                    end
    ...
    function kSlice(obj,varargin)
        
        modefreq = 1e9*input('Yarrrr me swashbucklin matlabeneers, what be yer mooodddeeee frequency, in GHz? ');
        fmrslice = obj.freq(:,round(size(obj.freq)/2),1);
        obj.kslice = sqrt(2*pi*(modefreq-fmrslice)/(4*pi*2.26627e5*930e3*(4.89e-9)^2));
        
        realk = real(obj.kslice);
        imagk = imag(obj.kslice);
        kplot = figure
        plot(1:size(obj.kslice,1),realk,1:size(obj.kslice,1),imagk)
        saveas(kplot, 'kslice.png');
    ...
    function show(obj,varargin)   
        ...
        % calculate scales
        xScale = linspace(obj.xmin,obj.xmax,obj.xnodes)*1e9;
        yScale = linspace(obj.ymin,obj.ymax,obj.ynodes)*1e9;
        ...
        f6 = figure(6);
            clf(); set(gcf,'name','DudeRanch','numbertitle','off');
            %imagesc(xScale,yScale,squeeze(obj.freq(:,:,params.zSlice)).'/1e9);
            surf(obj.freq(:,:,params.zSlice)/1e9);
            axis tight;
            title('Local FMR Frequency Surf'); 
            c1 = colorbar();
            c1.Label.String = 'Frequency (GHz)';
            xlabel('x (nm)');
            ylabel('y (nm)');
      ...
      %  if false
            f5 = figure(5);
                ...
                fslice = obj.freq(:,round(size(obj.freq,1)/2),1)/1e9;
                plot(xScale,squeeze(fslice),'k'); 
                title('Frequency');  
                axis xy;
                axis tight;
